...
RSpec.describe Shoryuken::SmartBatchWorkerRegistry do
  class SmartBatchRegistryTestWorker
  ...
  subject do
    Shoryuken.worker_registry = Shoryuken::SmartBatchWorkerRegistry.new
    Shoryuken.register_worker('default', TestWorker)
    Shoryuken.worker_registry
  end

  before do
    subject.register_worker 'registry-test', SmartBatchRegistryTestWorker
  end
  ...
  describe 'a registry with workers is handling messages' do
    ...
    context 'a batch of messages is being processed' do
      it 'returns an instance of the worker from the payload list' do
        batch = [build_message('default', SmartBatchRegistryTestWorker)]
        expect(subject.fetch_worker('default', batch)).to be_instance_of(SmartBatchRegistryTestWorker)
      end
    end
    ...
    context 'a single message is being processed' do
      context 'a worker class name is included in the message attributes' do
        it 'returns an instance of that worker' do
          message = build_message('default', SmartBatchRegistryTestWorker)
          expect(subject.fetch_worker('default', message)).to be_instance_of(SmartBatchRegistryTestWorker)
        end
      end
      ...
      context 'a worker class name is not included in the message attributes' do
        it 'returns an instance of the worker registered for that queue' do
          ...
          expect(subject.fetch_worker('registry-test', message)).to be_instance_of(SmartBatchRegistryTestWorker)
        end
      end
    end
  end
